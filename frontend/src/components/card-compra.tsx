		...
		<>
			{list.map((item, index) => (
				<Link href={`/produtos/${item.id}`} key={index}>
					<Card className="bg-[#333333] border-2 hover:scale-110 hover:duration-300 duration-300 flex w-full flex-col rounded-2xl overflow-hidden">
						...
						<CardFooter className="items-start flex-col px-5 py-2">
							...
							<div className="flex items-center justify-between w-full">
								...
								<p className="text-default-500 font-semibold text-[#fff] text-[19px]">
									{item.price}
								</p>
							</div>
						</CardFooter>
